...
class PrefixedStorage implements AutoCloseable {
  ...
  private Credentials credentials(GCPProperties properties) {
    ...
    } else if (properties.credentialsPath().isPresent()) {
      LOG.info(
          "Using Google credentials from path: {} for prefix {}",
          properties.credentialsPath().get(),
          storagePrefix);
      try (FileInputStream credentialsStream =
          new FileInputStream(properties.credentialsPath().get())) {
        return GoogleCredentials.fromStream(credentialsStream)
            .createScoped(StorageScopes.DEVSTORAGE_READ_WRITE);
      } catch (IOException e) {
        throw new UncheckedIOException(e);
      }
    } else if (properties.credentialsJson().isPresent()) {
      LOG.info("Using embedded Google credentials from configuration for prefix {}", storagePrefix);
      try (ByteArrayInputStream credentialsStream =
          new ByteArrayInputStream(
              properties.credentialsJson().get().getBytes(StandardCharsets.UTF_8))) {
        return GoogleCredentials.fromStream(credentialsStream)
            .createScoped(StorageScopes.DEVSTORAGE_READ_WRITE);
      } catch (IOException e) {
        throw new UncheckedIOException(e);
